...
! [[ -z "$runtype" ]]  &&  runtype=$runtype || runtype="PE"
...
! [[ -z "$mannotatorDB" ]]  &&  mannotatorDB=$mannotatorDB || mannotatorDB="''"
...
if [ $input_file ] ;then
    ...
    while IFS='\n' read -r line || [ "$line" ]; do
        ...
        stage_1 $base_dir ${scripts_dir} $pipeline_stage_start $pipeline_stage_end $runtype $app_dir
